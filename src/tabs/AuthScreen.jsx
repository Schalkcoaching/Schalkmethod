...
export default function AuthScreen({ onAuth }) {
  ...
  return (
    <div style={{
      display: 'flex', minHeight: '100vh', background: '#F7F3EE',
      alignItems: 'center', justifyContent: 'center',
    }}>
      {/* Left panel — dark branding */}
      <div style={{
        width: '420px', background: '#1C1917', minHeight: '100vh',
        display: 'flex', flexDirection: 'column', justifyContent: 'center',
        padding: '60px 48px',
      }}>
        <div style={{ marginBottom: '48px' }}>
          <div style={{ fontSize: '11px', color: '#5C5550', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '12px' }}>
            Welcome to
          </div>
          <div style={{ fontSize: '32px', fontWeight: 900, color: '#F7F3EE', lineHeight: 1.15, marginBottom: '16px' }}>
            The Schalk<br />Method
          </div>
        ...
        <div style={{ width: '100%', maxWidth: '380px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <button
              ...
              disabled={loading}
              style={{
                background: '#1C1917', border: 'none', borderRadius: '10px', padding: '14px',
                color: '#F7F3EE', fontSize: '14px', fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer',
          ...
          <div style={{ marginTop: '24px', display: 'flex', flexDirection: 'column', gap: '8px', alignItems: 'center' }}>
            {mode !== 'login' && (
              <button onClick={() => { setMode('login'); setError(''); setSuccess('') }} style={linkBtn}>
                Already have an account? Sign in
              </button>
            )}
            {mode !== 'signup' && (
              <button onClick={() => { setMode('signup'); setError(''); setSuccess('') }} style={linkBtn}>
                New client? Create account
              </button>
            )}
            {mode !== 'forgot' && (
              <button onClick={() => { setMode('forgot'); setError(''); setSuccess('') }} style={linkBtn}>
                Forgot password?
              </button>
            )}
          </div>
...
const inp = { background: '#FAFAF8', border: '1px solid #E0D8CE', borderRadius: '8px', padding: '11px 14px', color: '#1A1410', fontSize: '14px', outline: 'none' }
